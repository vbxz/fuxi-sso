...
public class UserServiceImpl extends ServiceImpl<UserDao, User, Integer> implements UserService {
    ...
    public Result login(String ip, String account, String password) {
        ...
        if (user == null) {
            ...
            login(user, ip, result);
        }
        return result;
    }

    private void login(User user, String ip, Result result) {
    ...
    public Result login(String ip, User user) {
        Result result = Result.createSuccessResult();
        login(user, ip, result);
        return result;
    }
    ...
    @Override
    public User findByWeiXinOpenId(String wxOpenId) {
        return dao.findByWeiXinOpenId(wxOpenId);
    }

    @Override
    public User findByWeiXinWebOpenId(String wxWebOpenId) {
        return dao.findByWeiXinWebOpenId(wxWebOpenId);
    }
}
