...
public class LoginController extends BaseController {
    ...
    public String login(
        ...
        return login(request, response, result, backUrl);
    }

    public String login(HttpServletRequest request, HttpServletResponse response, Result result, String backUrl) throws UnsupportedEncodingException {
        if (!result.getCode().equals(ResultCode.SUCCESS)) {
            ...
            // 如果原请求与SSO服务器不属于同一个顶级域名下，则需要在backUrl中追加token
            if (RequestUtils.inSameRootDomain(request, backUrl)) {
                backUrl = authBackUrl(backUrl, token);
            }
    ...
    private String goLoginPath(String backUrl, HttpServletRequest request) {
        ...
        String weiXinAuthorizeURL = WeiXinController.getWeiXinAuthorizeURL(request, backUrl);
        if (weiXinAuthorizeURL != null) {
            request.setAttribute("weiXinAuthorizeURL", weiXinAuthorizeURL);
        }
        return "/select_login_mode_m";
    }
    ...
    private void addTokenInCookie(String token, HttpServletRequest request, HttpServletResponse response) {
        ...
        String domain = RequestUtils.getRootDomain(request);
